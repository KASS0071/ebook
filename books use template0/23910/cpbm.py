...
def chinese_to_number(chinese):
    ...
    if '百' in chinese:
        ...
        if len(parts) == 2:
            hundreds = chinese_numbers.get(parts[0], 0)
            if parts[1] == '':
                return hundreds * 100
            else:
                return hundreds * 100 + chinese_to_number(parts[1])
...
def find_bookmarks(pdf_document):
    bookmarks = []
    pattern = re.compile(r"第([零一二三四五六七八九十百]+)回")
...
def generate_output_filename(input_filename):
    ...
    output_filename = f"{base}_with_bookmarks{ext}"
